...
def change_resolution(dataset: list, out_file_name: str, resolution="postcode6"):
    data = []
    missing = []
    postcodes_df = pd.read_csv(f"{dire}/../data/pc6hnr20210801_gwb.csv", sep=";")
    n = len(dataset)
    for i in dataset:
        n -= 1
        if n % 5 == 0:
            ...
            for y in x["address_components"]:
                if "postal_code" in y["types"]:
                    ...
                    if len(postal_code) == 4:
                        ...
                        print(f"postal_code 4:{postal_code}")
                    else:
...
if __name__ == "__main__":
    ...
    print(dire)
    dataset = load_data(f'{dire}/../data/merged_precise_coord.json')
    dataset = dataset[:10]
    ...
    postcodes_df = pd.read_csv(f"{dire}/../data/pc6hnr20210801_gwb.csv", sep=";")
    print(get_random_house("2324MC", "postcode6", postcodes_df))
