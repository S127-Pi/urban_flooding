...
resultFile = open(resultFolder + "result_precise_bouwjaar_wl_bouwjaar_2023.txt", "w+") 
column = 'layers'
def normalize(row):
        height = row[column]
        nans = height > 1000
...
listofarr = []
path = "precise_bouwjaar3.pkl"
df = pd.read_pickle(f"{begin_path}/pkl/{path}").reset_index()
df = df.dropna()
# df = df[["target", "bouwjaar", "date"]]
# df = df[["target", "layers", "past3hours", "bouwjaar"]]
df = df[["target", "lat", "lng", "layers", "past3hours"]]
